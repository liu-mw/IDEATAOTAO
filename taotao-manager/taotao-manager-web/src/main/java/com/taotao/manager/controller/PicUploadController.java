...
public class PicUploadController {
	...
	/*@Value("REPOSITORY_PATH")//spring（父容器）不能访问springMVC（子容器），子可以访问父容器@Value只是获取当前容器的值
	private String REPOSITORY_PATH;//因为这些配置文件在spring（父）中，虽然该picUploadController可以访问到子容器的值，
									//但是这个@value是只获取当前容器（springmvc)的值，所以访问不到，我们抽取到service层，让service层去获取当前
									//容器的值转而通过controller层直接访问到service层的配置文件达到间接访问的效果
									//在父容器中扫描包是com.taotao,子容器扫描包是com.taotao.manager.controller(这个绝对不能省略)
